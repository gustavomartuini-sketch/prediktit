...
// Full demo data - in production, replace with Prisma queries
...
export async function GET(request) {
  ...
  if (search) { const q = search.toLowerCase(); filtered = filtered.filter(m => m.title.toLowerCase().includes(q) || m.description.toLowerCase().includes(q)); }

  return NextResponse.json({ markets: filtered, total: filtered.length });
}
